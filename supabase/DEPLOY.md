...
| 항목 | 상태 |
|---|---|
| `gemini-proxy` v3 배포 | ✅ version 2 ACTIVE |
...
- **전체 일일 700 units** — 모든 응답이 상한을 채우는 최악 기준이라 실제로는 훨씬
  여유롭습니다. 워크플로우 생성(인터뷰+생성 = 3 units) 기준 하루 약 230회.
- **IP당 일일 40 units** — 생성 약 13회 또는 개선 2~3회.
